...
function CommissionsContent() {
  ...
  async function handleFiles(incoming: FileList | File[]) {
    ...
    try {
      // Step 1: Upload files (uses Vercel Blob when configured, direct upload otherwise)
      const uploadedFiles = await uploadMultipleFiles(fileArr, (msg) => setUploadProgress(msg));
      const uploaded = uploadedFiles.map(({ file, url }) => ({ file, blobUrl: url }));

      // Step 2: Ask the backend to run AI extraction on the uploaded files
      setUploadProgress(fileArr.length === 1 ? "Reading with AI…" : `Reading ${fileArr.length} files with AI…`);
      const res = await fetch(`${BASE}/api/commissions/process`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          period_id: selectedPeriodId,
          files: uploaded.map(({ file, blobUrl }) => ({
            blobUrl,
