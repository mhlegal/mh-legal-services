...
// Vercel bundles to CJS with esbuild — import.meta.url may be undefined.
// This pattern works in both native ESM and CJS-bundled environments.
...
// Lazy OpenAI client — do NOT instantiate at module load time.
// The SDK throws immediately when apiKey is undefined, which crashes the
// entire Vercel serverless function before any request is handled.
...
// ─── PROCESS FILES (called after direct browser→Vercel Blob upload) ──────────

router.post("/commissions/process", requirePaymentsAuth, async (req, res) => {
  const { period_id, files } = req.body as {
    period_id: number;
    files: Array<{ blobUrl: string; originalName: string; mimeType: string }>;
  };

  if (!period_id || !files || !files.length) {
    res.status(400).json({ error: "period_id and files are required" });
    return;
  }
  ...
  try {
    ...
    // Create statement records and download each file from GCS for AI processing
    const fileRecords = await Promise.all(files.map(async (file) => {
      const stmtResult = await query(
        ...
        [req.session.adminEmail, file.originalName, file.blobUrl, period.period_start, period.period_end, period_id]
      );
      const { stream } = await objectStorage.downloadObject(file.blobUrl);
      const chunks: Buffer[] = [];
      ...
      return { buffer, mimeType: file.mimeType, originalName: file.originalName, statementId: stmtResult.rows[0].id as number };
    }));

    // Extract entries — batch images in one AI call, PDFs separately
    const imageRecords = fileRecords.filter((r) => r.mimeType.startsWith("image/"));
    const pdfRecords   = fileRecords.filter((r) => r.mimeType === "application/pdf");
    ...
    if (imageRecords.length > 0) {
      const batchedResults = await extractWithAIFromImages(
        imageRecords.map((r) => ({ buffer: r.buffer, mimeType: r.mimeType }))
    ...
    if (pdfRecords.length > 0) {
      const pdfResults = await Promise.all(pdfRecords.map(async (r) => {
        const entries = await extractEntriesFromFile(r.buffer, r.mimeType, r.originalName).catch(() => []);
        return entries.map((e) => ({ ...e, statementId: r.statementId }));
      }));
      flatEntries.push(...pdfResults.flat());
    }

    // Deduplicate against DB and within this batch, then insert
    const batchPolicies = flatEntries.map((e) => e.policy_number?.trim()).filter((p): p is string => !!p);
    ...
    for (const entry of flatEntries) {
      ...
      if (policyNum && (existingPolicies.has(policyNum) || seenInBatch.has(policyNum))) { skippedCount++; continue; }
      ...
      try {
        ...
        if (insertErr?.code === "23505") { skippedCount++; } else { throw insertErr; }
      }
    }

    req.log.info({ fileCount: files.length, entryCount, skippedCount, period_id }, "Commission files processed from Vercel Blob");
    res.json({ success: true, entryCount, skippedCount, fileCount: files.length });
  } catch (err) {
    req.log.error({ err }, "Failed to process commission files");
