...
export async function sendPasswordReset(toEmail: string, resetURL: string) {
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#0a0a0a;color:#fff;padding:32px;border-radius:8px;">
      <div style="border-bottom:2px solid #C9A961;padding-bottom:16px;margin-bottom:24px;">
        <h1 style="color:#C9A961;margin:0;font-size:20px;">Password Reset — MH Legal Services</h1>
      </div>
      <p style="color:#ccc;margin-bottom:8px;">A password reset was requested for the manager portal.</p>
      <p style="color:#888;font-size:13px;margin-bottom:24px;">
        This link will set a new <strong>shared password</strong> for all managers. It expires in 15 minutes.
      </p>
      <a href="${resetURL}"
         style="display:inline-block;background:#C9A961;color:#000;font-weight:bold;text-decoration:none;padding:14px 28px;font-size:14px;letter-spacing:0.05em;text-transform:uppercase;">
        Reset Password
      </a>
      ...
      <p style="color:#444;font-size:12px;margin-top:16px;">
        If you did not request a password reset, ignore this email. No changes have been made.
      </p>
    </div>
  `;
  await sendMail(toEmail, "Manager Portal — Password Reset Request", html);
}
