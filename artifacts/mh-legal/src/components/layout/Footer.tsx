...
export function Footer() {
  ...
  return (
    <footer className="bg-foreground text-background py-16 md:py-24 border-t-4 border-accent">
      <div className="container mx-auto px-4 md:px-8 grid grid-cols-1 md:grid-cols-12 gap-12 lg:gap-20">

        <div className="md:col-span-5 space-y-6">
          <Link href="/">
            <div className="font-serif font-bold text-2xl tracking-wider cursor-pointer leading-tight">
              MHLOPHE HOLDINGS <br /><span className="text-accent">LEGAL SERVICES</span>
            </div>
          </Link>
          <p className="text-gray-400 text-lg max-w-md">{siteConfig.tagline}</p>
          <p className="text-gray-500 max-w-md leading-relaxed">{siteConfig.description}</p>
        </div>
        ...
        <div className="md:col-span-4 space-y-6">
          ...
          <ul className="space-y-4">
            <li>
              <a href={`mailto:${siteConfig.contact.company}`} className="text-gray-400 hover:text-white transition-colors flex items-start gap-3">
                <Mail size={18} className="text-accent mt-0.5 shrink-0" />
                <span>{siteConfig.contact.company}</span>
              </a>
            </li>
            <li>
